    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{jobs.length} application{jobs.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      <div className="flex gap-6 min-h-[500px]">
        {/* Table */}
        <div className={cn("flex-1 overflow-auto rounded-2xl border border-slate-100 bg-white shadow-sm", selectedJob && "lg:max-w-none")}>
          {loading ? (
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden sm:table-cell">Role</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:table-cell">Applied</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {jobs.map((job) => (
                  <tr
                    key={job.id}
                    onClick={() => selectJob(selectedJob?.id === job.id ? null : job)}
                    className={cn(
                      "cursor-pointer hover:bg-slate-50 transition-colors",
                      selectedJob?.id === job.id && "bg-indigo-50/50"
                    )}
                  >
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-900">{job.companyName}</div>
                    ...
                    <td className="px-4 py-3 text-slate-600 hidden sm:table-cell">{job.roleOrPosition}</td>
                    <td className="px-4 py-3">
                      <JobStatusBadge status={job.status} />
                    </td>
                    <td className="px-4 py-3 text-slate-500 hidden md:table-cell">{formatDate(job.dateApplied)}</td>
                    <td className="px-4 py-3 text-slate-500 hidden lg:table-cell">{job.location}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Add modal */}
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add Job Application" size="lg">
        <JobForm
