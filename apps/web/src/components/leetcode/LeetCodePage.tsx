...
import { SearchBar, Button, Select, EmptyState, StatCard } from "@/components/ui";
...
export function LeetCodePage() {
  ...
  function handleDifficultyFilter(value: string) {
    setFilters({ ...filters, difficulty: value ? [value as ProblemDifficulty] : undefined });
  }

  function handleStatusFilter(value: string) {
    setFilters({ ...filters, status: value ? [value as ProblemStatus] : undefined });
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">LeetCode Tracker</h1>
          <p className="text-sm text-slate-500 mt-0.5">{problems.length} problems</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard title="Total" value={problems.length} icon={<Code2 size={18} />} color="bg-indigo-500" />
        <StatCard title="Solved" value={solved} icon={<CheckCircle2 size={18} />} color="bg-green-500" />
        <StatCard title="Favorites" value={favorites} icon={<Star size={18} />} color="bg-yellow-500" />
        <StatCard
          title="Completion"
          value={problems.length > 0 ? `${Math.round((solved / problems.length) * 100)}%` : "0%"}
          color="bg-purple-500"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <SearchBar
          ...
          className="w-56"
        />
        <Select options={DIFFICULTY_FILTER} onChange={(e) => handleDifficultyFilter(e.target.value)} className="w-40" />
        <Select options={STATUS_FILTER} onChange={(e) => handleStatusFilter(e.target.value)} className="w-40" />
      </div>
      ...
      <div className="flex gap-6 min-h-[400px]">
        <div className="flex-1 overflow-auto rounded-2xl border border-slate-100 bg-white shadow-sm">
          {loading ? (
            <div className="py-16 flex items-center justify-center text-slate-400 text-sm">Loading…</div>
          ) : problems.length === 0 ? (
            <EmptyState
              icon={<Code2 size={22} />}
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden lg:table-cell">Revisions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {problems.map((problem) => (
                  <tr
                    key={problem.id}
                    onClick={() => selectProblem(selectedProblem?.id === problem.id ? null : problem)}
                    className={cn(
                      "cursor-pointer hover:bg-slate-50 transition-colors",
                      selectedProblem?.id === problem.id && "bg-indigo-50/50"
                    )}
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {problem.isFavorite && <Star size={12} className="text-yellow-400 fill-yellow-400 flex-shrink-0" />}
                        <span className="font-medium text-slate-900">{problem.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-slate-500 hidden sm:table-cell">{problem.type}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-slate-500 hidden lg:table-cell">{problem.revisionCount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Add modal */}
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add Problem" size="lg">
        <ProblemForm
